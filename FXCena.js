...
function getMesh(material, needsAnimatedColor = false) {
  const tamanho = 0.25;
  const geometria = new THREE.IcosahedronGeometry(tamanho, 1);
  const mesh = new THREE.InstancedMesh(geometria, material, objCount);
  ...
  for (let i = 0; i < objCount; i++) {
    ...
    if (needsAnimatedColor) { mesh.setColorAt(i, cor.setScalar(0.1 + 0.9 * Math.random())); }
  }
  return mesh;
}

export function getFXCena({ renderer, material, clearColor, needsAnimatedColor = false }) {
  ...
  cena.fog = new THREE.FogExp2(clearColor, 0.0002);

  cena.add(new THREE.HemisphereLight(0xffffff, 0x555555, 1.0));
